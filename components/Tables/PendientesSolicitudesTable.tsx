...
import LoadPendientes from "../../hooks/useLoadPendientes";
import LoadSolicitudesDPE from "../../hooks/useLoadSolicitudesDPE";

const PendientesSolicitudesTable = () => {
  ...
  useEffect(() => {
    ...
    async function llenarTabla() {
      const peticion = await LoadPendientes(activePage);
            ...
            data.length === 0 ? (
              ...
              <tbody>
                {data.map((user: any, i: number) => (
                  <>
                    {user.solicitudes_pendientes.length !== 0 ? (
                      <>
                        <tr
                          key={i}
                        ...
                        >
                          <th>{user.rut}</th>
                          <th>{user.name}</th>
                          <th>{user.solicitudes_pendientes.length}</th>
                          <th>
                            <Whisper
                              placement="top"
                              trigger="hover"
                          ...
                          <td colSpan={4}>
                            ...
                            >
                              <table className="table table-striped table-bordered">
                                ...
                                <tbody>
                                  {user.solicitudes_pendientes.map(
                                    (solicitud: any, z: any) => (
                                      <tr key={z}>
                                        <td>{solicitud.id}</td>
...
export default PendientesSolicitudesTable;
